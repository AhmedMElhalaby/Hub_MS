            ...
            <flux:navbar class="mr-1.5 space-x-0.5 py-0!">
                ...
                <flux:tooltip content="Notifications" position="bottom">
                    <flux:navlist.item icon="bell" :href="tenant_route('notifications.index')" :current="request()->routeIs('notifications.*')" class="relative">
                        ...
                        @endphp
                        @if($unreadCount > 0)
                            <span class="absolute right-1 top-1/2 -translate-y-2/3 flex items-center justify-center min-w-[20px] h-5 text-xs font-bold text-white bg-red-600 rounded-full px-1">
                                {{ $unreadCount }}
                            </span>
                        @endif
                    </flux:navlist.item>
