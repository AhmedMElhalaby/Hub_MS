...
$classes = Flux::classes()
    ->add(match ($variant) {
        ...
        'flyout' => match($position) {
            ...
            'left' => 'fixed m-0 p-8 max-h-dvh min-h-dvh md:[:where(&)]:min-w-[25rem] overflow-y-auto mr-auto [--fx-flyout-translate:translateX(-50px)] border-r rtl:border-l rtl:border-r-0',
            default => 'fixed m-0 p-8 max-h-dvh min-h-dvh md:[:where(&)]:min-w-[25rem] overflow-y-auto ml-auto [--fx-flyout-translate:translateX(50px)] border-l rtl:border-r rtl:border-l-0',
        },
        'flyout' => 'fixed m-0 p-8 max-h-dvh min-h-dvh md:[:where(&)]:min-w-[25rem] overflow-y-auto ml-auto rtl:mr-auto rtl:ml-0',
        'bare' => '',
    })
    ->add(match ($variant) {
...
<ui-modal {{ $attributes }} data-flux-modal>
    ...
    >
        ...
        <?php if ($closable): ?>
            <div class="absolute top-0 ltr:right-0 rtl:left-0 mt-4 ltr:mr-4 rtl:ml-4">
                <flux:modal.close>
